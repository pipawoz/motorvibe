...
class MotorListAdapter(private var motorList: MutableList<Motor>, private val gridMode: String, private val displayImages: Boolean,
                       val onItemClick: (Int) -> Boolean) : RecyclerView.Adapter<MotorListAdapter.MotorHolder>() {
    companion object {
    ...
    private var imagesList = listOf("motor1", "motor2", "motor3", "motor4", "motor5", "motor6", "motor7")
    private var modelList = listOf("W22", "IEEE 841", "Cooling Tower", "Crusher Duty", "W40", "Aegis 1", "JP-Aegis")
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MotorHolder {
        var view = LayoutInflater.from(parent.context).inflate(R.layout.item_motor_list, parent, false)

        if(gridMode == "grid_mode_grid") {
            view = LayoutInflater.from(parent.context).inflate(R.layout.item_motor_grid, parent, false)
        }
    ...
    override fun onBindViewHolder(holder: MotorHolder, position: Int) {
        ...
        val id = holder.getImageView().context.resources.getIdentifier(selectedImage, "drawable", holder.getImageView().context.packageName)
